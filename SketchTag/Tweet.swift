...
public class Tweet : CustomStringConvertible
{
    ...
    public struct IndexedKeyword: CustomStringConvertible
    {
        ...
        public var nsrange: NSRange? = nil            // index into an NS[Attributed]String made from the Tweet's text
        ...
        public init?(data: NSDictionary?, inText: String, prefix: String?) {
            ...
            if let startIndex = (indices?.firstObject as? NSNumber)?.integerValue {
             ...
             //               range = advance(inText.startIndex, start)...advance(inText.startIndex, end-1)
                           range = (inText.startIndex.advancedBy(start) )...(inText.startIndex.advancedBy(end-1) ) //Swift 2.0
...
//                                range = advance(inText.startIndex, start-1)...advance(inText.startIndex, end-2)
                                ...
                                range = (inText.startIndex.advancedBy(start-1) )...(inText.startIndex.advancedBy(end-2) ) //SWift 2.0
        ...
        /*
        /Users/keevinmitchell/Desktop/SketchTag/SketchTag/Tweet.swift:47:37: 'advance' is unavailable: call the 'advancedBy(n)' method on the index
        let startIndex = cardNumber.endIndex.advancedBy(-4)
        let lastFourDigitsOfPhoneNumber = phoneNumber.substringFromIndex(startIndex)
        
        /Users/keevinmitchell/Desktop/SketchTag/SketchTag/Tweet.swift:59:49: Variable 'self.nsrange' captured by a closure before being initialized
...
//            let range = rangeOfString(substring, options: NSStringCompareOptions.allZeros, range: NSMakeRange(start, end-start))
            ...
            let range = rangeOfString(substring as String, options: NSStringCompareOptions(), range: NSMakeRange(start, end-start)) //Swift 2.0
